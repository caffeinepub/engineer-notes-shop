...
actor {
  ...
  func validateOrTrap(_ : AccessControl.UserRole, isAdmin : Bool) : Bool {
    if (not isAdmin) { Runtime.trap("Unauthorized: Only admins can perform this action") };
    isAdmin;
  };
  ...
  // Purchase flow (authenticated users only)
  public shared ({ caller }) func purchaseProduct(productId : Text) : async () {
    ...
    let purchasedProductIds = switch (purchases.get(caller)) {
      case (null) {
        let newPurchasedProductIds = Set.singleton(productId);
        purchases.add(caller, newPurchasedProductIds);
        Set.empty<Text>();
      };
      case (?existingPurchases) {
        existingPurchases.add(productId);
        existingPurchases;
      };
    };
  };
  ...
  func hasUserPurchasedProduct(user : Principal, productId : Text) : Bool {
    switch (purchases.get(user)) {
      case (null) { false };
      case (?purchasedProductIds) {
